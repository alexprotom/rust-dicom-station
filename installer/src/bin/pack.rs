//! `rdv-pack` — turns the freshly built `rdv-setup.exe` into a shippable
...
//!     cd installer
//!     cargo build --release                 # rdv-setup.exe + rdv-pack.exe
//!     cargo run --release --bin rdv-pack    # dist/rust-dicom-viewer-setup.exe
//!
...
const USAGE: &str = "\
rdv-pack — build the shippable installer
...
OPTIONS:
    ...
    --app <FILE>       viewer executable (default: <repo>/target/release/rust-dicom-viewer.exe)
    --setup <FILE>     setup binary to wrap (default: target/release/rdv-setup.exe)
    --out <FILE>       installer to write (default: dist/rust-dicom-viewer-setup.exe)
...
fn main() -> Result<()> {
    ...
    let app = o
        .app
        .unwrap_or_else(|| o.repo.join("target/release/rust-dicom-viewer.exe"));
    let setup = o
        .setup
        .unwrap_or_else(|| crate_dir.join("target/release/rdv-setup.exe"));
    let out = o
        .out
        .unwrap_or_else(|| crate_dir.join("dist/rust-dicom-viewer-setup.exe"));
    ...
    let mut files: Vec<(String, PathBuf)> = vec![("rust-dicom-viewer.exe".into(), app.clone())];
