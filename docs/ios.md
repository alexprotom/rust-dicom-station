...
Rust DICOM Station runs on iPads as the same program it is on Windows,
Linux, macOS and Android: the same viewer, the same modules, the same
windows, compiled for `aarch64-apple-ios` and packaged as an `.ipa`. Nothing
was taken out and nothing was redrawn for touch; an iPad with a Magic
Keyboard, a mouse or an Apple Pencil gets the desktop program, and a finger
gets it too, with the notes below. The package is built by
...
iPhones are not a target. The three linked panes, the data tree and the
module panel want the room of a tablet; the bundle declares itself iPad-only
(`UIDeviceFamily` 2), so it does not install on a phone at all.
...
([Releasing](#releasing)). There are three ways onto an iPad:
...
| **Ad hoc** | The `.ipa` was signed with an ad hoc profile that lists the iPad's UDID | Until the profile expires (one year) |
...
An ad hoc build installs by dragging the `.ipa` onto the iPad in Finder
(macOS) or Apple Configurator, or with `xcrun devicectl device install app
--device <UDID> rust-dicom-station-<version>-ios.ipa`. iPadOS 16 and later
...
iPadOS 15 or later is required. Memory decides more than the chip: a 4DCT
study and the viewer's caches want an iPad with 4 GB or more.

## Getting studies onto the iPad

The program's data folder is its `Documents` folder, which the Files app shows
as **On My iPad > Rust DICOM Station** and Finder shows under the iPad's
*Files* tab when it is connected to a Mac. A DICOM folder copied there - from
iCloud Drive, a USB stick, a file server, an AirDrop - is an ordinary folder
to the program, and it is the first place the file browser offers
(*On My iPad*).
...
folder in iCloud Drive whose files have not been downloaded to the iPad yet
reads as empty; *Download Now* in the Files app first.
...
iPadOS ask whether the program may use it (*Local Network*, Settings >
Privacy & Security), and nothing gets through until that is allowed.

## Where the files are

iPadOS has no folders outside an app's own container, and gives each app a
...
| Models, archive, templates, test data | `~/Library/Application Support/RustDICOMStation` | `Documents` in the app's container, i.e. *On My iPad > Rust DICOM Station* in the Files app. `last_panic.txt` goes here too |
...
the iPad; the model manager shows the exact path. `Documents` is marked
...
written to `last_panic.txt` in `Documents`, so an iPad that has never seen a
Mac still keeps the evidence: open it in the Files app.
...
* **`src/main.rs`.** The entry point: the logger, the two folders, the panic
  ...
  `src/main.rs`, wrapped in a `Shell` that keeps the status bar and the home
  indicator free. `run_native` never returns on iOS: `winit` hands the main
  thread to UIKit.
* **`src/safe_area.rs`.** The strips iPadOS lays over the window. `egui-winit`
...
* **`src/places.rs`.** The system's folder picker
  ...
  what the file browser asks for places and for the picker.
* **`Info.plist.in`.** iPad only, iPadOS 15, all four orientations, the
...
* **`Assets.xcassets/`.** The app icon, the project's
  ...
  iPad reads. The 1024 px App Store icon is scaled up from the 256 px
...
* **`build-ipa.sh`.** `cargo build` for the device or the simulator, then the
  ...
  project.
* **`simulator-smoke.sh`.** Creates a fresh simulated iPad on the newest
  runtime, installs and starts the simulator build, and after 45 seconds
  saves a screenshot and fails if the program panicked or is no longer
  running.
...
* `settings.rs`: one arm each in `config_dir` and `data_dir`, and the
  `settings::ios::Places` registry the front end fills;
...
To run a build on your own iPad from a Mac, sign it with a development
profile; `xcrun devicectl` installs only signed apps:
...
* **ipa** and **simulator**, on a pull request that changes `packaging/ios/`
  ...
  `simulator` starts the simulator build on a simulated iPad and attaches the
  screenshot and the log as `ios-simulator`. For a release the simulator job
...
| `IOS_PROVISIONING_PROFILE_BASE64` | The provisioning profile for the app, base64-encoded: *Ad Hoc* to install on the iPads listed in it, *App Store* for TestFlight |
...
3. *Profiles*: an *Ad Hoc* profile for that App ID and the iPads to
...
* Opening a study from another app (the Files app's *Share*, *Open in*) is
  not wired up: the program reads folders, so copy the study into *On My
  iPad* or add its folder with *+ Folder from Files*.
...
* `winit` 0.30 runs the app through the application delegate rather than
  UIKit scenes. That is supported by iPadOS 26, but Apple has announced that
