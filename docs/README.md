...
| [motion-4d.md](motion-4d.md) | 4D sub-studies and the motion workflow: phase grouping (automatic + by hand), the per-phase registration/propagation pipeline, centroid and drift metrics with correlations, ITV generation, the results window with A/B comparison and CSV export, transfer by relationship, structure comparison (Dice, HD95) |
| [dvh.md](dvh.md) | Dose–volume histograms: what is sampled where, the axes, the metrics table, protocol constraint checking, CSV export, and the analytic phantom it is verified against |
