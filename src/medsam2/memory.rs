...
impl<B: Backend> CxBlock<B> {
    fn load(p: &Params, prefix: &str, dev: &B::Device) -> Result<CxBlock<B>> {
        Ok(CxBlock {
            ...
            pwconv1: Lin::load(p, &format!("{prefix}.pwconv1"), config::CX_MLP, D_MODEL, dev)?,
            pwconv2: Lin::load(p, &format!("{prefix}.pwconv2"), D_MODEL, config::CX_MLP, dev)?,
...
impl<B: Backend> MemoryEncoder<B> {
    pub fn load(p: &Params, dev: &B::Device) -> Result<MemoryEncoder<B>> {
        ...
        Ok(MemoryEncoder {
            ...
            pix_feat_proj: Conv::load_1x1(p, "memory_encoder.pix_feat_proj", D_MODEL, D_MODEL, dev)?,
            fuser,
