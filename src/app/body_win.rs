...
impl ViewerApp {
    ...
    /// The *Body contour* section of the Structure auto tools module; while
    /// a run is in flight its buttons become the progress row.
    pub(super) fn body_section(&mut self, ui: &mut egui::Ui) {
        let slot = self.auto.slot;
        ...
        let (mut run, mut browse, mut cancel) = (false, false, false);
        ui.label(
        ...
        ui.collapsing("Options", |ui| {
            ...
            ui.horizontal_wrapped(|ui| {
                ...
                .on_hover_text("A closing applied last, to take the staircase off the contour.");
            });
            if d.params.method == Method::ModelAssisted {
                ui.separator();
                ui.horizontal_wrapped(|ui| {
                ...
                browse = models_dir_row(ui, &mut self.models_dir, ModelsEngine::TotalSegmentator);
            }
        });
        ui.separator();
